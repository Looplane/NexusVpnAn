# ⚡ Simple Production Setup (Copy & Paste)
...
```bash
cd /opt/nexusvpn && wget -O /tmp/setup-prod.sh https://raw.githubusercontent.com/Looplane/NexusVpnAn/main/infrastructure/setup-production-simple.sh 2>/dev/null || curl -o /tmp/setup-prod.sh https://raw.githubusercontent.com/Looplane/NexusVpnAn/main/infrastructure/setup-production-simple.sh 2>/dev/null || cat > /tmp/setup-prod.sh << 'SCRIPT'
#!/bin/bash
set -e
GREEN='\033[0;32m'
BLUE='\033[0;34m'
...
## Alternative: Create Script File First

If the above doesn't work, create the script file first:

```bash
# Create the script file
cat > /opt/nexusvpn/infrastructure/setup-production-simple.sh << 'EOF'
#!/bin/bash
# Copy the entire content from infrastructure/setup-production-simple.sh
# (The file I just created)
EOF

# Make it executable
chmod +x /opt/nexusvpn/infrastructure/setup-production-simple.sh

# Run it
/opt/nexusvpn/infrastructure/setup-production-simple.sh
```

## What It Does

1. ✅ Configures backend with REAL DATA mode (MOCK_SSH=false)
2. ✅ Sets up frontend environment
3. ✅ Generates SSH keys for WireGuard management
4. ✅ Configures PM2 ecosystem

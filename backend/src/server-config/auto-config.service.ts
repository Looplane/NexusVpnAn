...
export class AutoConfigService {
  ...
  async autoConfigureServer(
    ...
    try {
      ...
      const osInfo = await this.detectionService.detectOS(ipv4, sshUser);
      ...
      const requirements = await this.detectionService.checkRequirements(ipv4, sshUser, osInfo);
      ...
      if (requirements.missingPackages.length > 0 || requirements.services.wireguard === 'not-installed') {
        ...
        await this.installRequirements(ipv4, sshUser, osInfo, requirements);
      ...
      if (!requirements.ssh || requirements.services.ssh !== 'running') {
        ...
        await this.configureSSH(ipv4, sshUser, osInfo);
      ...
      if (requirements.services.wireguard === 'not-installed' || requirements.services.wireguard === 'stopped') {
        ...
        await this.configureWireGuard(ipv4, sshUser, osInfo, serverData.wgPort || 51820);
      ...
      if (!requirements.firewall) {
        ...
        await this.configureFirewall(ipv4, sshUser, osInfo, serverData.wgPort || 51820);
      ...
      if (osInfo.type === 'linux' && !requirements.ipForwarding) {
        ...
        await this.enableIPForwarding(ipv4, sshUser);
        steps.push('IP forwarding enabled');
      }

      // Step 8: Get WireGuard public key
      steps.push('Fetching WireGuard public key...');
      const publicKey = await this.getWireGuardPublicKey(ipv4, sshUser, osInfo);
  ...
  private async installRequirements(
    ...
    if (osInfo.type === 'windows') {
      ...
      if (requirements.services.wireguard === 'not-installed') {
        ...
        await this.sshService.executeCommand(installCmd, ipv4, sshUser);
      }
      ...
      if (osInfo.distribution === 'ubuntu' || osInfo.distribution === 'debian') {
        await this.sshService.executeCommand('apt-get update -y', ipv4, sshUser);
        ...
        if (requirements.services.wireguard === 'not-installed') {
          await this.sshService.executeCommand('apt-get install -y wireguard wireguard-tools qrencode', ipv4, sshUser);
        }
   ...
   */
  private async configureSSH(ipv4: string, sshUser: string, osInfo: OSInfo): Promise<void> {
   ...
   * Configure WireGuard
   */
  private async configureWireGuard(ipv4: string, sshUser: string, osInfo: OSInfo, wgPort: number): Promise<void> {
    if (osInfo.type === 'windows') {
...
PersistentKeepalive = 25`;
      ...
      // Create config
      const privateKey = await this.sshService.executeCommand('cat /etc/wireguard/privatekey', ipv4, sshUser);
      const config = `[Interface]
...
PostDown = iptables -D FORWARD -i wg0 -j ACCEPT; iptables -t nat -D POSTROUTING -o eth0 -j MASQUERADE`;
   ...
   * Configure firewall
   */
  private async configureFirewall(ipv4: string, sshUser: string, osInfo: OSInfo, wgPort: number): Promise<void> {
    if (osInfo.type === 'windows') {
      // Windows Firewall
      await this.sshService.executeCommand(
   ...
   */
  private async enableIPForwarding(ipv4: string, sshUser: string): Promise<void> {
    await this.sshService.executeCommand(
   ...
   */
  private async getWireGuardPublicKey(ipv4: string, sshUser: string, osInfo: OSInfo): Promise<string> {
    if (osInfo.type === 'windows') {

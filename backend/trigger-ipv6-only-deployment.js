/**
 * IPv6-Only Deployment Trigger for Render
 * Triggers deployment with fixed configuration that works in IPv6-only environments
 * 
 * Date: 2024-12-15
...
const deploymentConfig = {
  clearCache: true,  // Clear build cache for clean deployment
  // No specific branch - use default branch
};
...
async function triggerIPv6Deployment() {
  ...
  try {
    ...
    const response = await makeAPIRequest('POST', `/v1/services/${RENDER_SERVICE_ID}/deploys`, deploymentConfig);
    ...
    console.log(`   🌐 Service URL: ${response.service?.serviceDetails?.url || "https://nexusvpn-api.onrender.com"}`);
    ...
    if (error.message.includes('429')) {
      console.log('\n🔍 Rate Limit Analysis:');
      console.log('📝 Too many deployment requests in a short time');
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      if (deploy.status === 'live') {
        ...
        console.log(`🌐 Live URL: ${deploy.service?.serviceDetails?.url || "https://nexusvpn-api.onrender.com"}`);
...
if (require.main === module) {
  console.log('🎯 IPv6-Only Deployment Trigger for NexusVPN');
